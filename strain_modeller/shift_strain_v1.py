...
from matplotlib.nxutiles import points_inside_poly
...
for reg in BIRD_GLOBAL_PARAMETERS:
...
def tapered_gutenberg_richter_cdf(moment, moment_threshold, beta, 
    ...
    cdf = exp((moment_threshold - moment) / corner_moment)
...
class Strain(object):
    ...
    def get_strain_data(self, input_strain_file=None):
        ...
        if not 'region' in header_names:
            ...
            self.strain_data['region'] = \
                [None for xval in range(0, self.number_points)]

            west_limit = np.min(self.strian_data['long'])
            east_limit = np.max(self.strian_data['long'])
            north_limit = np.max(self.strian_data['lat'])
            south_limit = np.min(self.strian_data['lat'])
            regionalisation = self.apply_kreemer_regionalisation(north_limit,
                south_limit, east_limit, west_limit)
            for polygon in kreemer_polygons:
                id0 = np.where(np.logical_and(
                    ...
                    self.strain_data['lat'] < polygon['lat_lims'][1])[0])
    ...
    def apply_kreemer_regionalisation(self, north=90., south = -90., 
        ...
        input_data = getlines('tectonic_areas.dat.txt')
        kreemer_polygons = []
    ...
    def continuum_seismicity(self, threshold_moment, e1h, e2h, err, 
        ...
        seismicity_rate = np.zeros(np.shape(strain_values)[0], dtype=float)
        ...
        arg_check = (region_params['CMT_moment'] - threshold_moment) /\
            region_params['corner_moment']
        id0 = arg_check >= -100.0
        
        seismicity_rate[id0] = ((threshold_moment / 
